...
# MAGIC %md
# MAGIC # Agent Safety & Alignment Dashboard
# MAGIC 
# MAGIC This notebook displays the Agent Safety & Alignment dashboard using displayHTML().
# MAGIC 
# MAGIC **Instructions:**
# MAGIC 1. Run this cell to display the dashboard
# MAGIC 2. To create a Databricks Dashboard: Click **View** → **Dashboards** → **Create Dashboard**
# MAGIC 3. You can refresh the dashboard by re-running this cell
...
<head>
    ...
    <style>
        ...
        .bar-purple { background-color: #D7DDE8; }
...
<body>
    ...
    <script>
        ...
        // Generate blocking rate bars
        function generateBlockingBars(agentId) {
            let bars = '';
            for (let i = 0; i < 30; i++) {
                let barClass = 'bar-purple';
                if (agentId === 'AG-01' && (i === 2 || i === 3)) {
                    barClass = 'bar-dark';
                }
                if (agentId === 'AG-02' && i === 22) {
                    barClass = 'bar-dark';
                }
                bars += `<div class="demand-bar ${barClass}"></div>`;
            }
            bars += `<div class="demand-bar bar-purple"></div>`;
            return bars;
        }
        ...
        cards.forEach(card => {
            ...
            const privacyIcon = `<svg class="w-4 h-4 mr-1.5 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="${fingerprintPath}"></path></svg>`;

            const privacyClass = card.privacy.type === 'warning' ? 'text-[#eab308] font-bold' : 'text-[#66cc00] font-bold';
            
            const percentageTextClass = 'text-black';
            const nowTextClass = 'text-black';

            const maliceVal = parseFloat(card.malice);
            let maliceClass;
            if (maliceVal < 2.2) maliceClass = 'text-black';
            ...
            let toxicityClass;
            if (toxicityVal < 2.2) toxicityClass = 'text-black';
            ...
            let groundingClass;
            if (groundingVal < 2.2) groundingClass = 'text-black';
            ...
            const html = `
                <div class="bg-white shadow-md border-2 border-gray-200 overflow-hidden flex flex-col transition-all duration-200 hover:shadow-xl hover:-translate-y-0.5 cursor-pointer">
                    ...
                    <div class="p-4 flex-grow">
                        <div class="flex items-center text-xs uppercase font-semibold tracking-wider text-black mb-4">
                        ...
                        <div class="flex flex-col mb-4">
                            <div class="text-xs font-semibold text-black uppercase tracking-wider mb-2">Demand Ability (Cognitive Load)</div>
                            <div class="flex items-center gap-1 min-w-0">
                                ...
                                <div class="flex flex-col items-start flex-shrink-0">
                                    ...
                                    <div class="text-[10px] font-semibold ${nowTextClass} uppercase tracking-wider mt-0.5 whitespace-nowrap">Load</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-start gap-4 mb-4">
                            <div class="flex flex-col items-center">
                                <div class="text-xs font-semibold text-black uppercase tracking-wider">Malice</div>
                                <div class="text-xl font-bold ${maliceClass} leading-none mt-1">${card.malice}</div>
                            </div>
                            <div class="flex flex-col items-center">
                                <div class="text-xs font-semibold text-black uppercase tracking-wider">Toxicity</div>
                                <div class="text-xl font-bold ${toxicityClass} leading-none mt-1">${card.toxicity}</div>
                            </div>
                            <div class="flex flex-col items-center">
                                <div class="text-xs font-semibold text-black uppercase tracking-wider">Grounding</div>
                                <div class="text-xl font-bold ${groundingClass} leading-none mt-1">${card.grounding}</div>
                            </div>
                        </div>

                        <div class="flex flex-col">
                            <div class="text-xs font-semibold text-black uppercase tracking-wider mb-2">Blocking Interventions</div>
                            <div class="flex items-center gap-1 min-w-0">
                                <div class="demand-bars flex-1 min-w-0">
                                ...
                                <div class="flex flex-col items-start flex-shrink-0">
                                    ...
                                    <div class="text-[10px] font-semibold ${nowTextClass} uppercase tracking-wider mt-0.5 whitespace-nowrap">Rate</div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="bg-gray-50/50 px-4 py-3 border-t border-gray-100 flex justify-between items-center text-xs font-semibold text-black relative">
